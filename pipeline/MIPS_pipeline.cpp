...
bitset<30> signextend30(bitset<16> immediate)
{
	string result;
	if (immediate[15] == 0) {
		result = "00000000000000" + immediate.to_string<char, std::string::traits_type, std::string::allocator_type>();
	}
	else {
		result = "11111111111111" + immediate.to_string<char, std::string::traits_type, std::string::allocator_type>();
	}
	return (bitset<30>(result));
}

bitset<32> signextend32(bitset<16> immediate)
{
	string result;
	if (immediate[15] == 0) {
		result = "0000000000000000" + immediate.to_string<char, std::string::traits_type, std::string::allocator_type>();
	}
	else {
		result = "1111111111111111" + immediate.to_string<char, std::string::traits_type, std::string::allocator_type>();
	}
...
struct IFStruct {
    ...
    bool        nop;
   	IFStruct(){
		nop = 0; //initialized to 0, all others are 1
	}
};
...
int main()
{
    ...
    while (1) {
        ...
        /* --------------------- WB stage --------------------- */
        if (state.WB.nop == 0){
			if (state.WB.wrt_enable == 1)// UPDATE RF
			{
        ...
        if (state.MEM.nop == 0){
            //operate
			if (state.WB.nop == 0 and state.WB.Wrt_reg_addr == state.MEM.Rt)
			{
       ...
       if (state.EX.nop == 0)
		{
			if (state.EX.wrt_enable == 1 && state.EX.is_I_type == 0 && state.EX.alu_op == 1)// add
			{
				...
				if (state.MEM.Wrt_reg_addr == state.EX.Rs && state.MEM.nop == 0 && state.MEM.wrt_enable == 1){// EX to EX forward
			...
			else if (state.EX.wrt_enable == 1 && state.EX.is_I_type == 1 && state.EX.alu_op == 1)// lw
			{
				if (state.WB.Wrt_reg_addr == state.EX.Rt && state.WB.nop == 0 && state.WB.wrt_enable == 1)
				{
					state.EX.Read_data2 = state.WB.Wrt_data;
				...
				newState.MEM.ALUresult = bitset<32>(state.EX.Read_data1.to_ulong() + signextend32(state.EX.Imm).to_ulong());
			}

			else if (state.EX.wrt_enable == 0 && state.EX.is_I_type == 1 && state.EX.alu_op == 1)// sw
			{
				if (state.WB.Wrt_reg_addr == state.EX.Rt && state.WB.nop == 0 && state.WB.wrt_enable == 1)
				{
					state.EX.Read_data2 = state.WB.Wrt_data;
        ...
        if (state.ID.nop == 0) //decode
		{
			...
			bitset<5> reg1 = bitset<5>(shiftbits(state.ID.Instr, 21));
			bitset<5> reg2 = bitset<5>(shiftbits(state.ID.Instr, 16));
			bitset<6> funct = bitset<6>(shiftbits(state.ID.Instr, 0));
			//bitset<1> ALUop = bitset<1>(string("0"));
            bitset<1> ALUop;
            newState.EX.Rs = reg1;
			newState.EX.Rt = reg2;
			...
			if (opcode.to_ulong() == 35 || opcode.to_ulong() == 43 || funct.to_ulong() == 33 || opcode.to_ulong() == 0x04) //lw, sw, addu, bne
			{
			...
			newState.EX.Read_data1 = myRF.readRF(reg1);
			newState.EX.Read_data2 = myRF.readRF(reg2);
			
			bitset<1> NEq = (myRF.readRF(reg1).to_ulong() != myRF.readRF(reg2).to_ulong()) ? 1 : 0;
			...
			if (IsBranch == bitset<1>("1") && NEq == bitset<1>("1"))// BNE
			{
		...
		{   
            ...
            if(state.EX.is_I_type == 1 && state.EX.rd_mem == 1 && state.EX.is_I_type == 1 && state.EX.rd_mem == 1 && bitset<1>(((bitset<6>(shiftbits(state.ID.Instr, 26))).to_ulong() == 43) ? 1 : 0) == bitset<1>("1"))
			{
				if (state.EX.Rt == bitset<5>(shiftbits(state.ID.Instr, 21)))
				{
					newState.ID = state.ID;
			...
			// bne
			if (bitset<1>((bitset<6>(shiftbits(state.ID.Instr, 26)).to_ulong() == 5) ? 1 : 0) == bitset<1>("1") && bitset<1>((myRF.readRF(bitset<5>(shiftbits(state.ID.Instr, 21))).to_ulong() == myRF.readRF(bitset<5>(shiftbits(state.ID.Instr, 16))).to_ulong()) ? 1 : 0) == bitset<1>("0"))
			{
				if (state.ID.nop == 0)
				{
					newState.ID.nop = 1;
        ...
        if (myInsMem.readInstr(state.IF.PC).to_string<char, std::string::traits_type, std::string::allocator_type>() != "11111111111111111111111111111111")
		{
			...
			{
				if (state.ID.nop == 0)
				{	auto signext = signextend32(newState.EX.Imm);
					...
					newState.IF.PC = bitset<32>(state.IF.PC.to_ulong()  + (bitset<32>((bitset<30>(shiftbits(signext, 0))).to_string<char, std::string::traits_type, std::string::allocator_type>() + "00")).to_ulong());
				}
				else
				{
